...
*A video explainer of the loop in this section using a similar dataset that goes through 2020. We now have data going through 2022!* {{< video src="https://www.youtube.com/watch?v=Ij9YWgUQg_s" >}}
...
-   We can continue to update the data to examine what happened. We don't yet have a lot of data on changes to the Court that have come about since Justice Ketanji Brown Jackson was confirmed following the retirement of Justice Breyer, but we can examine changes to the court median through 2022.

Let's load and explore our data.

-   `term`: is the year of the SC (1991-2022 except for 2005),
...
```{r, include=F}
justices <- read.csv("data/justices24.csv", stringsAsFactors = T)
```

```{r, eval=F}
justices <- read.csv("justices24.csv", stringsAsFactors = T)

## alternative
justices <- read.csv("https://raw.githubusercontent.com/ktmccabe/teachingdata/main/justices24.csv", stringsAsFactors = T)
```
